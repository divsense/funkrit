...
import { right, left, isRighht, isLeft, EitherT } from '../build/test/either.js'
import { RightPIO, LeftPIO, runEitherPIO } from '../build/test/either-pio.js'
...
test('EitherPIO :: map', async t => {
    ...
    const x1 = await runEitherPIO(e1)
    t.is(right(x1), 2)

    const x2 = await runEitherPIO(e2)
    t.is(right(x2), 4)

    const x3 = await runEitherPIO(e3)
    t.is(left(x3), 'null')

    const x4 = await runEitherPIO(e4)
    t.is(isLeft(x4), true)

    const x5 = await runEitherPIO(e5)
    t.is(isLeft(x5), true)
...
test('EitherPIO :: chain', async t => {
    ...
    const x5 = await runEitherPIO(e5).catch(x => t.is(x, 'a'))
    t.falsy(x5)

    const x6 = await runEitherPIO(e7).catch(x => t.is(x, 'rejected: 2'))
    t.falsy(x6)
